...
class Window1:
    ...
    def __init__ (self, master, root):
        ...
        titleBrowse     = LabelFrame     (content, bg= "Black", fg= "White", font=FontStyle.get(), width= 340, text="1. Selecione o arquivo")
        logBrowse       = ttk.Label      (titleBrowse, textvariable=self.cuttedFilename, background="black", foreground="white")
        logBtnBrowse    = StandardButton (frame= titleBrowse, text="Browse", command= self.browseLog)
        folderBrowse    = ttk.Label      (titleBrowse, textvariable=self.cuttedFoldername, background="black", foreground="white")
        fldBtnBrowse    = StandardButton (frame= titleBrowse, text="Browse", command= self.browseFolder)

        titleField      = LabelFrame     (content, bg= "Black", fg= "White", font=FontStyle.get(), text="2. Organize a ordem dos tipos de dados")
        ...
        titleSeparator  = LabelFrame     (content, bg= "Black", fg= "White", font=FontStyle.get(), text="3. Selecione o token de separação")
    ...
    def parse(self):
        ...
        with open(self.logFilename.get(), 'r') as logfile:
            ...
            if data == None:
                messagebox.showerror(title="Error", message="Could not match line structure to the log's [first] line")
                return None
